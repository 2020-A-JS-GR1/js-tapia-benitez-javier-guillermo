...
export class RutaListaFotografiasMascotasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    observableFotografiasMascotas
      .subscribe(
        ...
        error => {
          console.error('Error obteniendo fotografias de mascotas', error);
        }
      );
  ...
  eliminarFotografiaMascota(idFotografiaMascota: number) {
    const observableEliminarFotografiaMascota = this._fotografiaMascotaService.deleteFotografiaMascota(idFotografiaMascota);
    observableEliminarFotografiaMascota
      .subscribe(
        () => {
          const indice = this.arregloFotografiasMascotas.findIndex(fotografiaMascota => fotografiaMascota.id === idFotografiaMascota);
          this.arregloFotografiasMascotas.splice(indice, 1);
        },
        error => {
          console.error('Error eliminando fotografia de mascota', error);
        }
      );
  }
}
